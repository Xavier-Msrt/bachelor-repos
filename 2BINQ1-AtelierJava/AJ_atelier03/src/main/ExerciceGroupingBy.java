...
import static main.ExerciceGroupingBy.TransactionsLevel.*;
...
public class ExerciceGroupingBy {
    ...
    private void groupBy3() {
        ...
        Map<TransactionsLevel,List<Transaction>> s =
                transactions
                        .stream()
                        .collect(groupingBy(transaction -> {

                            int valeur = transaction.getValue();

                            if( valeur >= 1000) return VERY_HI ;
                            else if (valeur >= 800) return HI ;
                            else if (valeur >= 600) return ME ;
                            else  return LO;
                        }));
